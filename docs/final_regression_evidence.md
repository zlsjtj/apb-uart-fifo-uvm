...
正式回归使用 3 组基准 seed：101、201、301。每一组都执行 15 个用例，脚本会让用例 seed 在基准值上依次递增，因此实际覆盖的 seed 范围分别为 101–115、201–215 和 301–315，共 45 次仿真。
...
项目级交付验收使用 `scripts/run_acceptance.ps1`。它在正式回归之外，还会执行架构结构检查、错相异比时钟测试和三类故障注入，并生成 `reports/acceptance_summary.md`。

## 2. 结果

2026-09-03 在 P2 验证架构优化后的正式回归为 45/45 PASS。所有用例的 warning、error、fatal 均为 0。逐项结果见 `reports/final_regression/final_regression_summary.md`，每组 seed 的原始摘要也单独保存在同一目录。

45 个 UCDB 已统一合并。合并结果为：功能覆盖率 100%（65/65 个计划 bin）、断言覆盖率 100%（42/42 条均有有效尝试且无失败）、cover property 100%（14/14 命中）。详细文本报告位于 `reports/final_regression/coverage/`。
...
- `coverage/coverage_manifest.md`：45 个 UCDB 的合并清单；
...
- `reports/mutation_matrix.md`：三类故障注入的测试、seed、检出器和量化结果；
...
本轮一键验收为 4/4 步通过，错相异比时钟子集为 4/4 通过，三类 mutation 为 3/3 KILLED。这里的 mutation score 只覆盖预先选择的三种故障模型，不能外推为穷尽性故障覆盖。
