...
`endif
  ...
  apb_uart_regs u_regs (
    ...
    .tx_full(tx_full), .tx_empty(tx_empty), .rx_full(rx_full_pclk_q2),
    .rx_empty(rx_empty), .rx_fifo_rdata(rx_fifo_rdata),
    .rx_frame_err(rx_frame_err_pclk_q2), .irq(irq_o), .cfg_busy(cfg_busy),
    .prdata(prdata), .pslverr(pslverr), .ctrl_reg(ctrl_reg), .baud_reg(baud_reg),
  ...
  apb_uart_cfg_cdc u_cfg_cdc (
    .pclk(pclk), .pclk_rst_n(pclk_rst_n), .uart_clk(uart_clk),
  ...
  apb_uart_serial_core u_serial_core (
    ...
    .tx_o(tx_o), .tx_rd_en(tx_rd_en), .rx_data(rx_data), .rx_valid(rx_valid),
